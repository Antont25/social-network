...
const slice = createSlice({
  ...
  reducers: {
    ...
    setUserSubscription: ( state, action: PayloadAction<{ userId: number, isFollowing: boolean }> ) => {
...
export const fetchFollowUser = ( userId: number ): AppThunk => async dispatch => {
  try {
    dispatch(setUserSubscription({ userId, isFollowing: true }))
    let res = await api.followUser(userId)
    if (res.resultCode === 0) {
...
export const fetchUnFollowUser = ( userId: number ): AppThunk => async dispatch => {
  try {
    dispatch(setUserSubscription({ userId, isFollowing: true }))
    let res = await api.unFollowUser(userId)
    if (res.resultCode === 0) {
